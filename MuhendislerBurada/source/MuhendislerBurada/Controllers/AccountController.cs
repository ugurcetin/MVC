...
using MuhendislerBurada.Models;
using MuhendislerBurada.Data.Models;
using MuhendislerBurada.Model.Models;
using MuhendislerBurada.Service;
using MuhendislerBurada.Web.Mailers;
using MuhendislerBurada.Web.ViewModels;
using AutoMapper;
...
using MuhendislerBurada.ViewModels;
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            if (Request.QueryString["guid"] != null)
                MuhendislerBuradaSessionFacade.JoinGroupOrGoal = Request.QueryString["guid"];
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (MuhendislerBuradaSessionFacade.JoinGroupOrGoal != null)
                    {
                        string groupOrGoalJoinToken = MuhendislerBuradaSessionFacade.JoinGroupOrGoal;
        ...
        public ActionResult Followers(int page = 0)
        {
            ...
            /* followerlarin kullanici idleri alinmiyordu ? */
            var userList = users.ToList();
            int followerCount = followers.Count();
            var followerList = followers.ToList();
            for(int  i = 0; i < followerCount; i++)
            {
                followerList[i].UserId = userList[i].Id;
            }
            /**/
        ...
        public ActionResult Followings(int page = 0)
        {
            ...
            var followingList = followings.ToList();
            for (int i = 0; i < followingCount; i++)
            {
                followingList[i].UserId = userList[i].Id;
            }
            /**/
        ...
        public ActionResult Messages(string id) {
            ...
            var participantIds = privateMessageService.GetParticipants(User.Identity.GetUserId());

            if (participantIds.Count() == 0) {

                ViewBag.UserName = userService.GetUsers().FirstOrDefault(u => u.Id == id).UserName;
                ViewBag.Conversation = null;
                ViewBag.ParticipantIds = null;
                ViewBag.ParticipantNames = null;

                return View();

            }

            var participant = userService.GetUsers().FirstOrDefault(u => u.Id == id);

            if (id == null || participant == null) {
                
                string userId = participantIds.First();
                participant = userService.GetUsers().FirstOrDefault(u => u.Id == userId);
           
            }

            var messages = privateMessageService.GetConversation(User.Identity.GetUserId(), participant.UserName);

            ViewBag.ParticipantIds = participantIds.ToArray();
            ViewBag.ParticipantNames = userService.GetUserByUserId(participantIds).Select(u => u.UserName).ToArray();
            ViewBag.UserName = participant.UserName;
            ViewBag.Conversation = messages.Reverse();

            return View();
        }

        [HttpPost]
        public ActionResult Messages(PrivateMessageFormModel model) {

            if (ModelState.IsValid) {

                privateMessageService.SendMessage(User.Identity.GetUserId(), model.ReceiverUsername, model.Message);
                string receiverId = userService.GetUsers().First(u => u.UserName == model.ReceiverUsername).Id;
                return RedirectToAction("Messages/"+receiverId);
            }

            ModelState.AddModelError("", "Lutfen gerekli alanlari doldurunuz");
            ...
            if (participantIds.Count() == 0) {
                ...
                ViewBag.Conversation = null;
                ViewBag.Participants = null;

                return View();

            }
            string id = model.ReceiverUsername;
            var participant = userService.GetUsers().FirstOrDefault(u => u.UserName == id);
            string userId = null;

            if (id == null || participant == null) {

                userId = participantIds.First();
                id = userService.GetUsers().FirstOrDefault(u => u.Id == userId).UserName;

            }

            var messages = privateMessageService.GetConversation(User.Identity.GetUserId(), id);
